...
func TestGetNumMergeCell(t *testing.T) {
	tests := []struct {
		axis   string
		start  string
		end    string
	...
	for _, test := range tests {
		colNum, rowNum, err := f.GetNumMergeCell(sheet1, test.axis)
		assert.NoError(t, err)
		assert.Equal(t, test.colNum, colNum)
		assert.Equal(t, test.rowNum, rowNum)
	}
}
